...
export async function searchEverything(query: string) {
  ...
  const [tasks, projects, people, vendors, documents, meetings, decisions, risks] = await Promise.all([
    ...
    db.decision.findMany({ where: { decision: { contains: q } }, take: 5 }),
    db.risk.findMany({ where: { risk: { contains: q } }, take: 5 }),
  ]);

  return { tasks, projects, people, vendors, documents, meetings, decisions, risks };
}
