...
import { DockMagnifyContainer } from "./dock-magnify";
...
export function SidebarNav({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <>
      ...
      <DockMagnifyContainer className="flex flex-1 flex-col overflow-y-auto">
              ...
              {group.items.map((item) => {
                ...
                return (
                  <Link
                    ...
                    data-dock-item
                    className={cn(
                      "group flex items-center gap-2.5 rounded-md px-2 py-1.5 text-sm transition-colors will-change-transform",
        ...
        <div className="mt-4 flex flex-col gap-0.5 border-t pt-3" style={{ borderColor: "var(--border-subtle)" }}>
          <Link
            ...
            onClick={onNavigate}
            data-dock-item
            className="flex items-center gap-2.5 rounded-md px-2 py-1.5 text-sm transition-colors hover:bg-[var(--surface-hover)] will-change-transform"
            style={{
              color: pathname?.startsWith("/settings") ? "var(--route)" : "var(--foreground)",
              transformOrigin: "left center",
              transition: "transform 120ms ease-out, background-color 150ms",
            }}
          >
